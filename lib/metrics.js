...
function Metrics (mongoUrl, collection) {
    
    if(!(this instanceof Metrics)) {
        return new Metrics(mongoUrl, collection);
    }
    ...
    */
    this.aggregate = function aggregate(name, resolution, valueAggregator, sourceAggregator, query, callback) {
        ...
        if(!Metrics.AGGREGATORS[valueAggregator]) {
            return callback(new Error("valueAggregator is not supported"));
        }

        if(!Metrics.AGGREGATORS[sourceAggregator]) {
    ...
    /*
        ...
        for each source given in the @sourceFilter
        ...
        @param {Function} callback - callback function
            callback(err, results)
    */
    this.aggregateOnlyValues = function aggregate(name, resolution, valueAggregator, sourceFilter, query, callback) {
        ...
        if(!(sourceFilter instanceof Array)) {
            return callback(new Error("sourceFilter should be an array of sources"));
        }
        ...
        var aggreation = [
            {$match: filter},
            {$group: this._generateValueGrouping(resolution, valueAggregator)},
            {$sort: {"_id.source": 1, "_id.date.y": 1, "_id.date.mo": 1, "_id.date.d": 1, "_id.date.h": 1, "_id.date.m": 1, "_id.date.s": 1}}
        ];
